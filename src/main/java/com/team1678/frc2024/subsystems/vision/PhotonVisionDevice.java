        ...
        m_poseEstimator.update(result).ifPresent((estimation) -> {
            ...
            Pose3d[] tagPoses = (Pose3d[]) targets.stream()
                    .map((target) -> {
                        ...
                        Pose3d fieldToTag = fieldToRobot.transformBy(m_constants.robotToCamera).transformBy(cameraToTag);
                        return fieldToTag;
                    }).toArray();
            ...
            RobotState.getInstance().addVisionUpdate(new VisionUpdate(
                timestamp, robotPose2d.getTranslation(), new Translation2d(), xyStdDev));
